...
const currentDir = path.resolve(new URL((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))).pathname, '..');
...
class CrossChainApi {
    ...
    async userBurn(smgId, toAddress, tokenPair, amount) {
        ...
        const tokenPair_0 = BigInt(tokenPair);
        const amount_0 = BigInt(amount);
        coinInfo(ledger.nativeToken(), amount_0);
...
// }
const coinInfo = (token, value) => require$$0.encodeCoinInfo(ledger.createCoinInfo(token, value));
