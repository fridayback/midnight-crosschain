 ...
 * @LastEditTime: 2025-11-25 16:15:46
...
export class CrossChainApi {
  ...
  async init(config: Config, contractAddress: ContractAddress) {
  ...
  async getFeeCommonConfig(chainId: bigint | string | number, targetLedger: CrossChain.Ledger | undefined) {
    let ledger;
    if (targetLedger) {
      ledger = targetLedger;
    } else {
  ...
  async getLedgerState(): Promise<CrossChain.Ledger | null> {
    if (!this.provider) throw 'provider is null';
    if (!this.crossChainContract) throw 'crossChainContract is null';
